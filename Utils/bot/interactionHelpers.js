 ...
 */
async function createInputModal(interaction, modalData, onSubmit, timeout = 300000) {
    try {
        await interaction.showModal(modalData.modal);
        
        const filter = i => i.user.id === interaction.user.id;
        const submission = await interaction.awaitModalSubmit({ filter, time: timeout })
            .catch(() => null);
 ...
 */
async function cleanupMessages(messages, delay = 5000, filter = msg => msg.deletable && msg.attachments.size === 0) {
    setTimeout(async () => {
