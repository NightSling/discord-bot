...
const {
  ...
  cleanupMessages,
} = require('../../Utils/bot/interactionHelpers');
  ...
  async execute(interaction) {
    ...
    try {
      ...
      const evidenceEmbed = new EmbedBuilder()
        ...
        .setDescription(
          [
            '**Attach your evidence files now**',
            '- Max 8 files (8MB each)',
            '- Supported: PNG, JPG, MP4, TXT',
          ...
          for (const attachment of m.attachments.values()) {
            if (attachment.size > 8_388_608) {
              await sendTemporaryMessage(
                modalInteraction,
                `❌ ${attachment.name} exceeds 8MB limit`,
              );
              continue;
            }
      ...
      evidenceCollector.on('end', async (collected, reason) => {
        try {
          ...
          await sendTemporaryMessage(modalInteraction, null, {
            embeds: [confirmation],
          });
