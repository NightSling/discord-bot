...
module.exports = {
  ...
  async execute(interaction) {
    ...
    try {
        ...
        .addFields(
          ...
          },
          {
            ...
            value: org.code_of_conduct
              ? `[View Here](${org.code_of_conduct.url})`
              : 'Not provided',
            inline: true,
          },
          {
            name: 'Social Media',
            value: org.twitter_username
              ? `[Twitter](https://twitter.com/${org.twitter_username})`
              : 'Not provided',
            inline: true,
          },
      ...
      const filter = (i) =>
        i.customId === 'select-repo' && i.user.id === interaction.user.id;
      ...
      collector.on('collect', async (i) => {
        ...
        const repoEmbed = new EmbedBuilder()
          ...
          .addFields(
            ...
            {
              name: 'GitHub URL',
              value: `[Visit Here](${selectedRepo.html_url})`,
              inline: true,
            },
          );
      ...
      collector.on('end', async () => {
        ...
        await message.edit({
          content:
            'Time expired. Please use the command again to view repository details.',
          components: [disabledRow],
        });
      });
    } catch (error) {
      console.error(
        'Error fetching organization or repository info:',
        error.response ? error.response.data : error.message,
      );
      await interaction.editReply({
        content:
          'Unable to fetch organization or repository details. Please check if the organization is public or the GitHub token is valid.',
        flags: 64,
      });
    }
  },
