...
module.exports = {
  ...
  async execute(message, args) {
    ...
    if (isNaN(numMessages) || numMessages < 1 || numMessages > 100) {
      return message
        .reply('Please provide a number between 1 and 100.')
        .then((msg) => setTimeout(() => msg.delete().catch(() => {}), 5000));
    }
    ...
    try {
      ...
      if (messagesToDelete.size === 0) {
        return message
          .reply(
            'No messages found to delete, or messages are too old (>14 days).',
          )
          .then((msg) => setTimeout(() => msg.delete().catch(() => {}), 5000));
      }
      ...
      const embed = new EmbedBuilder()
        ...
        .setDescription(
          `Are you sure you want to delete ${messagesToDelete.size} messages in this channel?\n\n > **Note: ⚠️ Due to Discord policy, I cannot delete messages older than 14 days.**`,
        )
        .setFooter({
      ...
      const row = new ActionRowBuilder().addComponents(
        new ButtonBuilder()
          .setCustomId(`confirm_purge_${authorId}`)
          .setLabel('Yes')
          .setStyle(ButtonStyle.Danger),
        new ButtonBuilder()
          .setCustomId(`cancel_purge_${authorId}`)
          .setLabel('Nevermind')
      ...
      const filter = (i) =>
        (i.customId === `confirm_purge_${authorId}` ||
          i.customId === `cancel_purge_${authorId}`) &&
        i.user.id === authorId;
      ...
      collector.on('collect', async (i) => {
        ...
        if (i.user.id !== authorId) {
          return i.reply({
            content:
              'Only the user who initiated this command can interact with these buttons.',
            flags: 64,
          });
        }
        ...
        if (i.customId === `confirm_purge_${authorId}`) {
          try {
            ...
            const successEmbed = new EmbedBuilder()
              ...
              .setDescription(
                `Successfully deleted ${deleted.size} messages in this channel.`,
              )
              .setFooter({
                text: `Command run by ${message.author.tag}`,
                iconURL: message.author.displayAvatarURL(),
            ...
            await i.update({
              content:
                'There was an error trying to delete messages. Some messages might be too old.',
              embeds: [],
              components: [],
            });
          }
          ...
          await i.update({
            content: 'Purge operation canceled.',
            embeds: [],
            components: [],
          });
        }
